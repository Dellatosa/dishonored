...
Hooks.once("init", function() {
    let versionInfo;
    versionInfo = game.world.coreVersion;
    ...
    game.settings.register("dishonored", "multipleComplications", {
    ...
    game.settings.register("dishonored", "send2ActorPermissionLevel", {
    ...
    game.settings.register("dishonored", "chaosPermissionLevel", {
    ...
    game.settings.register("dishonored", "momentumPermissionLevel", {
    ...
    game.settings.register("dishonored", "maxNumberOfExperience", {
    ...
    game.settings.register("dishonored", "trackerRefreshRate", {
    ...
    game.settings.register("dishonored", "ignoreTrackerPermissionsAlert", {
        scope: "world",
        type: Boolean,
        default: false,
        config: true
    ...
    game.settings.register("dishonored", "chaos", {
        scope: "world",
        type: Number,
        default: 0,
        config: false
    ...
    game.settings.register("dishonored", "momentum", {
        scope: "world",
        type: Number,
        default: 0,
        config: false
    ...
    game.settings.register("dishonored", "currentMigrationVersion", {
        scope: "world",
        type: String,
        default: 0,
        config: false
    ...
    Hooks.on("ready", function() {
        var currentMigVer = game.settings.get("dishonored", "currentMigrationVersion") == 0 ? "0.1.0": game.settings.get("dishonored", "currentMigrationVersion");
        if (isNewerVersion(game.system.version, currentMigVer ?? "0.1.0")) {
        ...
        let i;
        let error;
        i = foundry.CONST.USER_ROLES[game.settings.get("dishonored", "momentumPermissionLevel")];
        ...
        i = foundry.CONST.USER_ROLES[game.settings.get("dishonored", "chaosPermissionLevel")];
        for (i; i <= 4; i++) {
            if (!game.permissions.SETTINGS_MODIFY.includes(i)) {
                error = "chaos";
            }
        ...
        if (error) {
            ...
            string += error == "momentum" ? game.settings.get("dishonored", "momentumPermissionLevel") : game.settings.get("dishonored", "chaosPermissionLevel");
            ...
            if (game.settings.get("dishonored", "ignoreTrackerPermissionsAlert") == false && game.user.isGM ) {
                string +="</br><span style=\"font-weight: bold; color: #ffaaaa; text-decoration: underline; cursor: pointer;\" onclick=\"game.settings.set('dishonored', 'ignoreTrackerPermissionsAlert', 'true');\"> ";
        ...
        renderTemplate("systems/dishonored/templates/apps/tracker.html").then(function() {
            t.render(true);
        });
        let l = new DishonoredLogo();
        renderTemplate("systems/dishonored/templates/apps/logo.html").then(function() {
            l.render(true);
        });
    });
        ...
        /*case "0.4.1":
            ...
            game.actors.forEach(function(actor) {
                if (actor.data.type == "character") {
                    if (actor.data.data.truth1) actor.createOwnedItem({name:actor.data.data.truth1, type: "truth", img:"systems/dishonored/icons/dishonoredDefaultLogo.webp"});
                    if (actor.data.data.truth2) actor.createOwnedItem({name:actor.data.data.truth2, type: "truth", img:"systems/dishonored/icons/dishonoredDefaultLogo.webp"});
                }
            });
            game.settings.set(dishonored", "currentMigrationVersion", "0.5.0");
            currentMigVer = "0.5.0";
            recheck = true;
            break;
        case "0.5.0":
            ...
            game.settings.set("dishonored", "currentMigrationVersion", "0.5.1");
            currentMigVer = "0.5.1";
            recheck = true;
            break;
        case "0.5.1":
            ...
            game.actors.forEach(function(actor) {
                if (actor.img == "icons/svg/mystery-man.svg" || actor.img == "systems/dishonored/icons/dishonoredDefaultLogo.webp" ) {
                    actor.update({"img": "/systems/dishonored/icons/dishonoredDefaultLogo.webp"});
                }
                actor.data.items.forEach(function(item) {
                    if (item.img == "icons/svg/mystery-man.svg" || item.img == "icons/svg/item-bag.svg" || item.img == "/systems/dishonored/icons/dishonoredlogo.webp" ) {
                        item.update({"img": "/systems/dishonored/icons/dishonoredDefaultLogo.webp"});
                    }
                });
            });
            game.settings.set("dishonored", "currentMigrationVersion", "0.5.2");
            currentMigVer = "0.5.2";
            recheck = true;
            break;*/
        default:
