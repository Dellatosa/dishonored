export class DishonoredTracker extends Application {
    ...
    static get defaultOptions() {
        ...
        options.template = "systems/dishonored/templates/apps/tracker.html";
    ...
    activateListeners(html) {
        ...
        let chaos = game.settings.get("dishonored", "chaos");
        let momentum = game.settings.get("dishonored", "momentum");
        ...
        if (!game.user.hasRole(game.settings.get("dishonored", "chaosPermissionLevel"))) {
        ...
        html.find("#dishonored-chaos-track-increase").click(function() {
            ...
            if (game.user.hasRole(game.settings.get("dishonored", "chaosPermissionLevel"))) {
                ...
                momentum = game.settings.get("dishonored", "momentum");
                ...
                chaos = chaos + 1;
                game.settings.set("dishonored", "chaos", chaos);
                renderTracker();
            }
        ...
        html.find("#dishonored-track-chaos").change(function() {
            ...
            if (game.user.hasRole(game.settings.get("dishonored", "chaosPermissionLevel"))) {
                ...
                momentum = game.settings.get("dishonored", "momentum");
                chaos = game.settings.get("dishonored", "chaos");
        ...
        html.find("[id^=\"dishonored-momentum-tracker\"]").click(ev => {
            ...
            if (newTotalObjectBG.getAttribute("data-selected") === "true") {
                ...
                if (!html.find("#"+nextCheck)[0] || html.find("#"+nextCheck)[0].getAttribute("data-selected") != "true") {
                    momentum = momentum - 1;
                    game.settings.set("dishonored", "momentum", momentum);
                    renderTracker();
                } 
    ...
    async checkUpdates() {
        ...
        let refreshRate = game.settings.get("dishonored", "trackerRefreshRate") * 1000;
        ...
        setInterval(function check() {
            ...
            let storedChaos = game.settings.get("dishonored", "chaos");
            let storedMomentum = game.settings.get("dishonored", "momentum");
            // Get out clause that checks if the chaos or momentum is below 0 and if so, change it to 0. 
            if (storedChaos < 0) {
                game.settings.set("dishonored", "chaos", 0);
            }
            if (storedMomentum < 0) {
                game.settings.set("dishonored", "momentum", 0);
            }
